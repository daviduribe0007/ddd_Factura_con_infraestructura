...
public class FacturaMaterialize {
    ...
    @Async
    @EventListener
    public void handleEventPropinaCalculada(PropinaCalculada propinaCalculada) {
        logger.info("****** Handle event propina calculada");
        Update update2 = new Update();
        update2.set("propina", propinaCalculada.getPropina().value());
        mongoTemplate.updateFirst(getFilterByAggregateId(propinaCalculada), update2, COLLECTION_NAME);
    }

    @Async
    @EventListener
    public void handleEventTotalCalculado(TotalCalculado totalCalculado) {
        logger.info("****** Handle event total calculado");
        Update update2 = new Update();
        update2.set("total", totalCalculado.getTotal().value());
        mongoTemplate.updateFirst(getFilterByAggregateId(totalCalculado), update2, COLLECTION_NAME);
    }

    @Async
    @EventListener
    public void handleEventResenaAgregada(ResenaAgregada resenaAgregada) {
        logger.info("****** Handle event resena agregada");
        Update update = new Update();
        update.set("resena", resenaAgregada.getResena().value());
        mongoTemplate.updateFirst(getFilterByAggregateId(resenaAgregada), update, COLLECTION_NAME);
    }

    @Async
    @EventListener
    public void handleEventProductoRemovido(ProductoRemovido productoRemovido) {
        logger.info("****** Handle event producto eliminado");
        Update update = new Update();
        update.unset("producto."+ productoRemovido.getProductoId().value());
        mongoTemplate.updateFirst(getFilterByAggregateId(productoRemovido), update, COLLECTION_NAME);
    }
